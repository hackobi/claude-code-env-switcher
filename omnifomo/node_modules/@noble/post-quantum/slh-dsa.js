...
const base2b = (outLen, b) => {
    const mask = (0, utils_ts_1.getMask)(b);
    return (bytes) => {
...
function gen(opts, hashOpts) {
    ...
    const hashMsgCoder = (0, utils_ts_1.splitCoder)(Math.ceil((A * K) / 8), Math.ceil(TREE_BITS / 8), Math.ceil(TREE_HEIGHT / 8));
    const hashMessage = (R, pkSeed, msg, context) => {
        ...
        const leafIdx = Number(bytesToNumberBE(tmpIdxLeaf)) & (0, utils_ts_1.getMask)(LEAF_BITS);
    ...
    const seedCoder = (0, utils_ts_1.splitCoder)(N, N, N);
    const publicCoder = (0, utils_ts_1.splitCoder)(N, N);
    const secretCoder = (0, utils_ts_1.splitCoder)(N, N, publicCoder.bytesLen);
    const forsCoder = (0, utils_ts_1.vecCoder)((0, utils_ts_1.splitCoder)(N, N * A), K);
    const wotsCoder = (0, utils_ts_1.vecCoder)((0, utils_ts_1.splitCoder)(WOTS_LEN * N, TREE_HEIGHT * N), D);
    const sigCoder = (0, utils_ts_1.splitCoder)(N, forsCoder, wotsCoder); // random || fors || wots
    const internal = {
        signRandBytes: N,
        keygen(seed) {
            seed = seed === undefined ? (0, utils_ts_1.randomBytes)(seedCoder.bytesLen) : seed.slice();
            ...
            context.clean();
            (0, utils_ts_1.cleanBytes)(secretSeed, secretPRF, root, wotsAddr, topTreeAddr);
        ...
        sign: (sk, msg, random) => {
            ...
            if (!random)
                random = pkSeed.slice();
            (0, utils_ts_1.ensureBytes)(random, N);
            ...
            (0, utils_ts_1.cleanBytes)(R, random, treeAddr, wotsAddr, forsLeaf, forsTreeAddr, indices, roots);
            return SIG;
        },
        verify: (publicKey, msg, sig) => {
            ...
            return (0, utils_ts_1.equalBytes)(root, pubRoot);
        },
    };
    return {
        internal,
