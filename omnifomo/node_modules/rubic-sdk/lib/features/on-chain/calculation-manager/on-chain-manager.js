...
const provider_default_options_1 = require("./providers/dexes/common/on-chain-provider/constants/provider-default-options");
const on_chain_manager_aggregators_types_1 = require("./models/on-chain-manager-aggregators-types");
const assert_evm_token_1 = require("./utils/assert-evm-token");
/**
...
class OnChainManager {
    ...
    async calculateTradeFromTokens(from, to, options) {
        if ((from.isNative && to.isWrapped) || (from.isWrapped && to.isNative)) {
            (0, assert_evm_token_1.assertEvmToken)(from);
            return [
                {
    ...
    async calculateLifiTrade(from, to, options) {
        try {
            ...
            const calculationOptions = {
                ...options,
                slippageTolerance: options?.slippageTolerance || provider_default_options_1.providerDefaultOptions.slippageTolerance,
    ...
    static async getWrapTrade(from, to, _options) {
        const fromToken = from;
        const toToken = to;
        const ZERO_FEE_ADDRESS = '0x51c276f1392E87D4De6203BdD80c83f5F62724d4';
        let hasProxyContract;
        let proxyFeeInfo = undefined;
        try {
            proxyFeeInfo = await this.onChainProxyService.getFeeInfo(from, ZERO_FEE_ADDRESS);
            hasProxyContract = true;
        }
        catch {
            hasProxyContract = false;
        }
        const fromWithoutFee = (0, get_from_without_fee_1.getFromWithoutFee)(from, proxyFeeInfo?.platformFee.percent);
    ...
    getWrappedWrapTrade(fromToken, toToken, fullOptions) {
        (0, assert_evm_token_1.assertEvmToken)(fromToken);
        return (0, rxjs_1.from)(OnChainManager.getWrapTrade(fromToken, toToken, fullOptions)).pipe((0, rxjs_1.map)(wrapTrade => ({
            total: 1,
            calculated: 1,
            wrappedTrade: {
                error: undefined,
...
OnChainManager.onChainProxyService = new on_chain_proxy_service_1.OnChainProxyService();
