...
class IzumiProvider extends evm_on_chain_provider_1.EvmOnChainProvider {
    ...
    async calculate(from, to, options) {
        if (this.config?.tokenBlackList) {
            const isDisabledToken = this.config.tokenBlackList?.some(disabledToken => {
                ...
                if ((0, blockchain_1.compareAddresses)(tokenAddress, disabledToken.tokenAddress)) {
                    return true;
                }
                return false;
            });
        ...
        const searchParams = {
            ...
            shortBatchSize: this.blockchain === blockchain_name_1.BLOCKCHAIN_NAME.MERLIN ? 5 : 20
        };
        ...
        if (options?.gasCalculation === 'calculate') {
            try {
                ...
                tradeStruct.gasFeeInfo = (0, get_gas_fee_info_1.getGasFeeInfo)(gasPriceInfo, {
                    ...(gasLimit && { gasLimit })
                });
            }
            catch { }
    ...
    getProviderTrade(tradeStruct, providerAddress) {
        return new izumi_trade_1.IzumiTrade(tradeStruct, providerAddress);
    }
}
