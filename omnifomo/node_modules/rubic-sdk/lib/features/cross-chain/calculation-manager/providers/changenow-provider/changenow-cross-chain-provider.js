...
class ChangenowCrossChainProvider extends cross_chain_provider_1.CrossChainProvider {
    ...
    async calculate(from, toToken, options) {
        ...
        if (!fromCurrency) {
            ...
            onChainTrade = await this.getOnChainTrade(fromWithoutFee, [], options.slippageTolerance, transitCurrency
                ? transitCurrency.tokenContract
                : native_tokens_1.nativeTokensList[fromBlockchain].address);
            if (!onChainTrade) {
                return {
                    trade: null,
                    error: new errors_1.NotSupportedTokensError(),
        ...
        try {
            const { toAmount, quoteError } = await this.fetchQuoteData(transit, toCurrency, fromWithoutFee);
            ...
            const trade = new changenow_cross_chain_trade_1.ChangenowCrossChainTrade(changenowTrade, options.providerAddress, await this.getRoutePath(from, to), useProxy);
            if (quoteError) {
                return {
                    trade,
                    error: quoteError,
    ...
    async fetchQuoteData(fromCurrency, toCurrency, fromWithoutFee) {
        try {
            ...
            return {
                toAmount: new bignumber_js_1.default(res.toAmount)
            };
        }
        catch (err) {
            const error = err?.error;
            if (error?.message?.includes('Out of min amount')) {
                const minAmount = new bignumber_js_1.default(error?.payload?.range?.minAmount);
                return {
                    toAmount: new bignumber_js_1.default(0),
                    quoteError: new errors_1.MinAmountError(minAmount, fromWithoutFee.symbol)
                };
            }
