    ...
    async batchMulticallTokenBalance(userAddress, tokensAddresses, batchSize) {
        if (tokensAddresses.length > 100) {
            ...
            for (let i = 0; i < tokensAddresses.length; i += batchSize) {
                ...
                const batchResult = await this.multicallContractsMethods(this.tokenContractAbi, batchRequest.map(tokenAddress => ({
        ...
        return this.multicallContractsMethods(this.tokenContractAbi, tokensAddresses.map(tokenAddress => ({
            contractAddress: tokenAddress,
            methodsData: [
                {
                    methodName: 'balanceOf',
