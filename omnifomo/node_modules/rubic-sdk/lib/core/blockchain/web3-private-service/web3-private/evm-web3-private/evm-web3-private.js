...
const uni_v3_permit2_abi_1 = require("../../../web3-public-service/web3-public/evm-web3-public/constants/uni-v3-permit2-abi");
...
class EvmWeb3Private extends web3_private_1.Web3Private {
    ...
    async trySendTransaction(toAddress, options) {
        try {
            ...
            const gasfulParams = {
                ...gaslessParams,
                ...(0, options_1.getGasOptions)(options),
                gas: web3_private_1.Web3Private.stringifyAmount(gas, options?.gasLimitRatio || 1.05)
            };
            try {
    ...
    async shouldIgnoreError(error) {
        if (error.message === 'Low native value' &&
            (await this.getBlockchainName()) === blockchain_name_1.BLOCKCHAIN_NAME.MANTLE) {
            return true;
        }
     ...
     */
    async approveOnPermit2(tokenAddress, permit2Address, spenderAddress, amount = 'infinity', deadline = new bignumber_js_1.default(1000000), options = {}) {
        const contract = new this.web3.eth.Contract(uni_v3_permit2_abi_1.UNI_V3_PERMIT_2_ABI, permit2Address);
        const rawValue = amount === 'infinity' ? new bignumber_js_1.default(2).pow(256).minus(1) : amount;
        const gaslessParams = { from: this.address };
        const expiration = new bignumber_js_1.default(Date.now()).plus(deadline).toFixed();
        const gas = await contract.methods['approve'](tokenAddress, spenderAddress, rawValue.toFixed(), expiration).estimateGas();
        const gasfullParams = {
            ...gaslessParams,
            ...(0, options_1.getGasOptions)(options),
            gas: web3_private_1.Web3Private.stringifyAmount(gas, 1)
        ...
        return new Promise((resolve, reject) => {
            contract.methods['approve'](tokenAddress, spenderAddress, rawValue.toFixed(), expiration)
                .send(gasfullParams)
                .on('receipt', resolve)
                .on('error', (err) => {
                console.error(`Tokens approve error. ${err}`);
    ...
    async simulateTransaction(toAddress, options, blockchain) {
        try {
            const web3 = this.web3 ?? injector_1.Injector.web3PublicService.getWeb3Public(blockchain).web3Provider;
            const gaslessParams = {
                from: this.address,
                to: toAddress,
                value: web3_private_1.Web3Private.stringifyAmount(options.value || 0),
            ...
            const gas = await web3.eth.estimateGas(gaslessParams);
            const gasfulParams = {
                ...gaslessParams,
                ...(0, options_1.getGasOptions)(options),
                gas: web3_private_1.Web3Private.stringifyAmount(gas, options?.gasLimitRatio || 1.05)
            ...
            try {
                await web3.eth.estimateGas(gasfulParams);
                return gasfulParams;
            }
            catch {
                throw new errors_1.RubicSdkError('Low native value');
