...
let TronWeb3Pure = TronWeb3Pure_1 = class TronWeb3Pure {
    ...
    static addressToHex(address) {
        return tronweb_1.TronWeb.address.toHex(address).replace(/^41/, '0x');
    }
    /**
    ...
    static encodeFunctionCall(contractAbi, methodName, methodArguments) {
        ...
        const encodedMethodSignature = tronweb_1.TronWeb.sha3(`${methodSignature.name}(${this.flattenTypesToString(methodSignature.inputs).join(',')})`).slice(0, 10);
        const encodedParameters = tronweb_1.utils.abi.encodeParams(this.flattenTypesToArray(methodSignature.inputs), methodArguments);
    ...
    static encodeMethodSignature(methodSignature, parameters) {
        ...
        const encodedParameters = tronweb_1.utils.abi.encodeParams(flattenedParameters[0], flattenedParameters[1]);
        return encodedMethodSignature + encodedParameters.slice(2);
    }
    /**
    ...
    static decodeMethodOutput(outputAbi, response) {
        const decodedParam = tronweb_1.utils.abi.decodeParams([], this.flattenTypesToArray(outputAbi), response)[0];
    ...
    static flattenParameterToPrimitive(parameter) {
        if (typeof parameter === 'number' ||
            parameter instanceof ethers_1.BigNumber ||
