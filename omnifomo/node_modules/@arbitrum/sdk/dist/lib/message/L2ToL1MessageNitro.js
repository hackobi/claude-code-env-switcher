...
class L2ToL1MessageReaderNitro extends L2ToL1MessageNitro {
    ...
    async getBlockFromAssertionLog(l2Provider, log) {
    ...
    async getBlockFromAssertionId(rollup, assertionId, l2Provider) {
        ...
        return await this.getBlockFromAssertionLog(l2Provider, logs[0]);
    }
    ...
    async getSendProps(l2Provider) {
        if (!this.sendRootConfirmed) {
            ...
            const l2BlockConfirmed = await this.getBlockFromAssertionId(rollup, latestConfirmedAssertionId, l2Provider);
            ...
            if (sendRootSizeConfirmed.gt(this.event.position)) {
                ...
                if (!latestEquals) {
                    ...
                    const l2Block = await this.getBlockFromAssertionId(rollup, latestCreatedAssertionId, l2Provider);
     ...
     * @returns
     */
    async isBold(l2Network, l1Provider) {
        const bridge = Bridge__factory_1.Bridge__factory.connect(l2Network.ethBridge.bridge, l1Provider);
        const remoteRollupAddr = await bridge.rollup();
        const rollup = RollupUserLogic__factory_1.RollupUserLogic__factory.connect(remoteRollupAddr, l1Provider);
        try {
     ...
     */
    async getRollupAndUpdateNetwork(l2Network) {
        if (!l2Network.isBold) {
        ...
        return l2Network.isBold
            ? BoldRollupUserLogic__factory_1.BoldRollupUserLogic__factory.connect(l2Network.ethBridge.rollup, this.l1Provider)
            : RollupUserLogic__factory_1.RollupUserLogic__factory.connect(l2Network.ethBridge.rollup, this.l1Provider);
    }
    /**
    ...
    async getFirstExecutableBlock(l2Provider) {
        ...
        const lastL2Block = logs.length === 0
            ? undefined
            : await this.getBlockFromAssertionLog(l2Provider, logs[logs.length - 1]);
        ...
        while (left <= right) {
            ...
            const log = logs[mid];
            const l2Block = await this.getBlockFromAssertionLog(l2Provider, log);
