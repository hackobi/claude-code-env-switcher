...
class Erc20Bridger extends assetBridger_1.AssetBridger {
     ...
     * @returns
     */
    async isRegistered({ erc20L1Address, l1Provider, l2Provider, }) {
        const l1StandardGatewayAddressFromChainConfig = this.l2Network.tokenBridge.l1ERC20Gateway;
        const l1GatewayAddressFromL1GatewayRouter = await this.getL1GatewayAddress(erc20L1Address, l1Provider);
        // token uses standard gateway; no need to check further
        if (l1StandardGatewayAddressFromChainConfig.toLowerCase() ===
            l1GatewayAddressFromL1GatewayRouter.toLowerCase()) {
            return true;
        }
        const tokenL2AddressFromL1GatewayRouter = await this.getL2ERC20Address(erc20L1Address, l1Provider);
        const l2GatewayAddressFromL2Router = await this.getL2GatewayAddress(erc20L1Address, l2Provider);
        const l2AddressFromL2Gateway = await L2ERC20Gateway__factory_1.L2ERC20Gateway__factory.connect(l2GatewayAddressFromL2Router, l2Provider).calculateL2TokenAddress(erc20L1Address);
        return (tokenL2AddressFromL1GatewayRouter.toLowerCase() ===
...
class AdminErc20Bridger extends Erc20Bridger {
    ...
    async approveGasTokenForCustomTokenRegistration(params) {
        ...
        await this.checkL1Network(params.l1Signer);
        const approveGasTokenRequest = this.isApproveParams(params)
            ? this.getApproveGasTokenForCustomTokenRegistrationRequest(Object.assign(Object.assign({}, params), { l1Provider: signerOrProvider_1.SignerProviderUtils.getProviderOrThrow(params.l1Signer) }))
            : params.txRequest;
        return params.l1Signer.sendTransaction(Object.assign(Object.assign({}, approveGasTokenRequest), params.overrides));
    }
    /**
    ...
    async registerCustomToken(l1TokenAddress, l2TokenAddress, l1Signer, l2Provider) {
        ...
        if (!this.nativeTokenIsEth) {
            const nativeTokenContract = ERC20__factory_1.ERC20__factory.connect(this.nativeToken, l1Provider);
            const allowance = await nativeTokenContract.allowance(l1SenderAddress, l1Token.address);
            const maxFeePerGasOnL2 = (await l2Provider.getFeeData()).maxFeePerGas;
            const maxFeePerGasOnL2WithBuffer = this.percentIncrease(maxFeePerGasOnL2, ethers_1.BigNumber.from(500));
            // hardcode gas limit to 60k
            const estimatedGasFee = ethers_1.BigNumber.from(60000).mul(maxFeePerGasOnL2WithBuffer);
            if (allowance.lt(estimatedGasFee)) {
                throw new Error(`Insufficient allowance. Please increase spending for: owner - ${l1SenderAddress}, spender - ${l1Token.address}.`);
            }
        }
    ...
    async getL2GatewaySetEvents(l2Provider, filter, customNetworkL2GatewayRouter) {
        ...
        const eventFetcher = new eventFetcher_1.EventFetcher(l2Provider);
        return (await eventFetcher.getEvents(L2GatewayRouter__factory_1.L2GatewayRouter__factory, t => t.filters.GatewaySet(), Object.assign(Object.assign({}, filter), { address: l2GatewayRouterAddress }))).map(a => a.event);
    }
    /**
