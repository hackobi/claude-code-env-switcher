...
class Demos {
    ...
    constructor() {
        ...
        this.web2 = {
            ...
            getDiscordMessage: async (discordUrl) => {
    ...
    newMnemonic(strength = 128) {
        return bip39.generateMnemonic(english_js_1.wordlist, strength);
    }
    /**
     ...
     */
    async connectWallet(masterSeed, options) {
        if (!masterSeed) {
        ...
        if (typeof masterSeed === "string") {
            ...
            if (!bip39.validateMnemonic(masterSeed, english_js_1.wordlist)) {
     ...
     */
    async verifyMessage(message, signature, publicKey, options) {
        const algorithm = options?.algorithm || this.algorithm;
        let messageBuffer = null;
        if (typeof message === "string") {
            messageBuffer = new TextEncoder().encode(message);
     ...
     */
    _calculateAndApplyGasFee(raw_tx) {
    ...
    async rpcCall(request, isAuthenticated = false, retries = 0, sleepTime = 250, allowedErrorCodes = []) {
        let publicKey = "";
        let signature = "";
        if (isAuthenticated) {
        ...
        try {
            ...
            if (retries > 0) {
                ...
                return await this.rpcCall(request, isAuthenticated, retries - 1, sleepTime, allowedErrorCodes);
            }
            return response.data;
        }
        catch (error) {
    ...
    async call(method, message, data = {}, extra = "", sender = null, receiver = null) {
    ...
    async getLastBlockNumber() {
        return (await this.nodeCall("getLastBlockNumber"));
    }
    /**
     * Get the last block hash.
     */
    async getLastBlockHash() {
        return (await this.nodeCall("getLastBlockHash"));
    }
    /**
     * Get list of blocks.
     *
     ...
     */
    async getTxByHash(txHash = "e25860ec6a7cccff0371091fed3a4c6839b1231ccec8cf2cb36eca3533af8f11") {
     ...
     */
    async getTransactionHistory(address, type = "all", options = {}) {
