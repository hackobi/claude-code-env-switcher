...
class Identities {
     ...
     */
    async getDemosIdsByIdentity(demos, identity) {
        const request = {
            method: "gcr_routine",
     ...
     * @returns The account associated with the identity.
     */
    async getDemosIdsByWeb2Identity(demos, context, username, userId) {
     ...
     * @returns The account associated with the identity.
     */
    async getDemosIdsByWeb3Identity(demos, chain, address) {
        return await this.getDemosIdsByIdentity(demos, {
            type: "xm",
            chain: chain,
            address: address,
     ...
     */
    async getDemosIdsByTwitter(demos, username, userId) {
     ...
     * @returns The account associated with the identity.
     */
    async getDemosIdsByGithub(demos, username, userId) {
     ...
     * @returns The account associated with the identity.
     */
    async getDemosIdsByDiscord(demos, username, userId) {
     ...
     * @returns The account associated with the identity.
     */
    async getDemosIdsByTelegram(demos, username, userId) {
     ...
     */
    generateUDChallenge(demosPublicKey, signingAddress) {
     ...
     */
    async resolveUDDomain(demos, domain) {
    ...
    async addUnstoppableDomainIdentity(demos, signingAddress, signature, challenge, resolutionData, referralCode) {
        ...
        const isOwner = resolutionData.metadata?.[signatureType]?.owner === signingAddress;
        if (!isOwner) {
            const isAuthorized = resolutionData.authorizedAddresses.some(auth => auth.address === signingAddress);
            if (!isAuthorized) {
                throw new Error(`Can't sign payload. Signing address (${signingAddress} on ${signatureType}) is not the owner or an authorized address`);
            }
        }
     ...
     * @returns The identities associated with the address.
     */
    async getUDIdentities(demos, address) {
